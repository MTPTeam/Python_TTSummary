...
def TTS_TDSWTT(path, mypath = None):
    ...
    try:
        ...
        masterofstations = {y: x for x,y in stationmaster.items()}
        ...
        vys_D = zip_stations(['Varsity Lakes TB','Varsity Lakes platform','Varsity Lakes','Robina platform','Robina','Robina Stabling Yard','Nerang','Helensvale arrive','Helensvale depart','Coomera arrive','Coomera depart','Ormeau','Beenleigh arrive','Beenleigh platform','Beenleigh depart','Loganlea','Altandi','Tennyson','Moolabin','Park Road platform','Park Road','South Bank','South Brisbane','Roma St West Junction','Roma St Fork','Mayne North','Mayne Yard Arrival','Mayne Junction','Elec Train S','Campbell St','Exhibition','Normanby','Roma Street arrive','Roma Street platform','Roma Street depart','Central arrive','Central platform','Central depart','Fortitude Valley arrive','Fortitude Valley platform','Fortitude Valley depart','Bowen Hills arrive','Bowen Hills platform','Bowen Hills depart','Mayne','Albion','Wooloowin','Eagle Junction platform','Eagle Junction','Airport Junction','International Airport','Domestic Airport'])
        vys_U = zip_stations(['Domestic Airport','International Airport','Airport Junction','Eagle Junction platform','Eagle Junction','Wooloowin','Albion','Mayne','Bowen Hills arrive','Bowen Hills platform','Bowen Hills depart','Fortitude Valley arrive','Fortitude Valley platform','Fortitude Valley depart','Central arrive','Central platform','Central depart','Roma Street arrive','Roma Street platform','Roma Street depart','Normanby','Roma St Fork','Exhibition','Campbell St','Elec Train S','Roma St West Junction','Moolabin','Tennyson','South Brisbane','South Bank','Park Road platform','Park Road','Altandi','Loganlea','Beenleigh arrive','Beenleigh platform','Beenleigh depart','Ormeau','Coomera arrive','Coomera depart','Helensvale arrive','Helensvale depart','Nerang','Robina Stabling Yard','Robina platform','Robina','Varsity Lakes platform','Varsity Lakes','Varsity Lakes TB'])
        ...
        with open(path) as f:
            for index, line in enumerate(f):
                if line.startswith('TT'):
                    pass
                elif line.startswith('STRT'):
                    daycode = re.findall('STRT\s+\S{4}\s+(\d+)', line)[0]
                    day = DoO_dict.get(daycode)
                    if day not in d_list:
                        d_list.append(day)    
        
        WEEK_ORDER = ['64','32','16','8','120','4','2','1'] 
        d_list.sort(key=WEEK_ORDER.index)
        ...
        #     origin = train[1][0].attrib
        #     destin = train[1][-1].attrib
        #     # stops = train[1].findall('entry')
        ...
        #         pass
        #     else:
        #         forms_dict[(  train[1][0][0].attrib['trainNumber'],   WeekdayKey    )] = (  tn   ,   odep     ,   oID     )
        
        
        
        def write_workbook(days, weekdaykeys):
            """ 
            ...
            def build_triplist(triplist,line,Outbound=False):
                ...
                # stations = {x.attrib['stationID'] for x in train[1].iter('entry')}
                
                # origin = train[1][0].attrib
                # destin = train[1][-1].attrib
                # stops = train[1].findall('entry')
                # stationIDs = [x.attrib['stationID'] for x in stops]
                # oID = origin['stationID']
                # dID = destin['stationID']
                
                
                condition = set(stationIDs).intersection(line_stops) 
                ...
                if line == 'Beenleigh':
                    condition = condition and set(stationIDs).isdisjoint(uniquestations_dict.get('Varsity Lakes - Airport'))
                ...
                if condition:
                    ...
                    tripdict['DoO'] = dayname_dict.get(WeekdayKey)
                    tripdict['Run ID'] = '-'
                    tripdict['Unit'] = unit
                    tripdict['Formed By'] =  prev_connect
                    tripdict['Formed At'] = odep
                    tripdict['Formed/Origin Location'] = oID
                    tripdict['Forms'] = next_connect
                    tripdict['Forms At'] =  ddep
                    tripdict['Forms/Destination Location'] = dID
                    # tripdict['Run ID'] = train.attrib['lineID'].split('~',1)[1][1:]
                    ...
                    # else:
                    #     tripdict['Formed By'] = train[1][0][0].get('trainNumber')
                    #     tripdict['Formed At'] = train[1][0][0].get('trainDeparture')
                    ...
                    for entry in entries:
                        ...
                        tripdict[stationID+'pfm'] = platform_number
                        ...
                        for i,rtd in enumerate(refinedtriplist):
                            if end_idx - i <= n:
                                ...
                                for key in tripdict_relevant_keys:
                                    ...
                                    # if rtd.get('DoO') not in ['M-Th','Sun','Sat','Fri','M-F','WE']:
                                    #     print(rtd.get('DoO'))
            ...
            def write_timetable(sheet):   
                ...
                # len_tn = []
                # for x in triplist:
                #     if x['Train ID'] not in len_tn:
                #         len_tn.append(x['Train ID'])
                # print(len(len_tn))
                ...
                for i,x in enumerate(triplist,1):
                    ...
                    # if x.get('Train ID') in ['EP29','J501']:
                    #     print(vals)
                    #     print()
                        ...
                        if v and ':' in v:
                            ...
                            if temp_vals and prev_vals:
                                if temp_vals < prev_vals:
                                    # print(x.get('Train ID'))
                                    # if x.get('Train ID') in ['EP29']:
                                    #     print(v,uvals,temp_vals)
                                    if s in mainstations:
            ...
            stationstring = '#\d+\s+([A-Z]+)'
            stationplatformstring = '#\d+\s+([A-Z]+\d+)'
            platformstring = '#\d+'
            tnstring = 'STRT\s+(\S{4})'
            timecode = '\s+(\d{10})'
            connectionstring = '\s+(\w{4})\s+'
            ...
            with open(path) as f:
                for index, line in enumerate(f):
                    ...
                    if line.startswith('TT'):
                        ...
                        tn = re.findall(tnstring,line)[0]
                        # revenue = tn[0] in ['1','D','J','T','U','X']
                        daycode = re.findall('STRT\s+\S{4}\s+(\d+)', line)[0]
                        DoO = DoO_dict.get(daycode) in weekdaykeys
            ...
            for x in trains:
                entries = []
                stationplatforms = re.findall(stationplatformstring, x)
                
                
                stationIDs = re.findall(stationstring, x)
                s_long = [masterofstations.get(s) for s in stationIDs]
                tn = re.findall(tnstring,x)[0]
                ...
                # tIDs = re.findall(connectionstring,x)
                # print(tn)
                # print(tIDs)
                # # tn = tIDs[0]
                # if len(tIDs) >= 2:
                ...
                stops = []
                entries = [] #!!!
                for i,y in enumerate(splitlist):
                    if y.startswith('STRT'):
                        # print(y)
                        # print('   ',tn)
                        # print(re.findall('STRT\s+\S{4}\s+(\d+)', y))
                        daycode = re.findall('STRT\s+\S{4}\s+(\d+)', y)[0]
                        WeekdayKey = DoO_dict.get(daycode)
                        tIDs = re.findall(connectionstring,y)
                        # if tn == '1E07':
                        #     print(tIDs)
                        # tn = tIDs[0]
                        ...
                        prev_connect = tIDs[1] if len(tIDs) >= 2 else 'FNSH'
                        next_connect = tIDs[2] if len(tIDs) >= 3 else 'STRT'
                        
                        
                        
                    elif y.startswith('END') or len(y) == 0:
                        pass
                    else:
                        # print(y)
                        
                        platform_number = re.findall(platformstring, y)[0]
                        
                        tds_stables_converter = {
                            'EMH3': 'EMHS',
                            'EMH4': 'EMHS',
                            'KR10': 'KPRS',
                            'IPS5': 'IPSS',
                        ...
                        sID = re.findall(stationplatformstring, y)
                        # print(tn,sID)
                        sID = sID[0]
                        sID = tds_stables_converter.get(sID,sID)
                        sID = ''.join(filter(str.isalpha, sID))
                        
                        
                        arrdep = re.findall(timecode, y)[0]
                        arrive = arrdep[:5]
                        depart = arrdep[5:]
                        arr = time_reformat(arrive)
                        dep = time_reformat(depart)
        
                        # stationtype = 'stop' if bool(re.search('\d{10}(D)',y)) or bool(re.search('\d{10}(E)',y)) else 'pass'
                        stationtype = 'stop' if bool(re.search('\d{10}(D)',y)) else 'pass'
                
                        if i == 1:
                            oID = sID
                            odep = dep
                        elif bool(re.findall(stationstring, y)):
                            dID = re.findall(stationstring, y)[0]
                            ddep = dep
                        ...
                        entries.append([sID,arr,dep,stationtype])
                        # print([sID,arr,dep,stationtype])
                # print(entries)
                # print()
            ...
            if OpenWorkbook:
                dayofop = dayofop_dict.get(book)
                os.startfile(rf'{dayofop[0]}')
