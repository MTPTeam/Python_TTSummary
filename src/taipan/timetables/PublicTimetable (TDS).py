...
def TTS_TDSPTT(path, mypath = None):
    ...
    try:
        ...
        dayname_dict = {'120':'M-Th', '4':'Fri', '2':'Sat', '1':'Sun'}
        ...
        def write_workbook(days, weekdaykeys):
            ...
            def build_triplist(triplist, line, Outbound=False):
                """ """
                cbd = 'IPS' if line == 'Ipswich - Rosewood' else 'BNC'
                ...
                condition = set(stops).intersection(line_stops)
                
                if line == 'Inner North':
                    condition = condition and set(stops).intersection(['NTG','EGJ'])
                ...
                if condition:
                    ...
                    for n,entry in enumerate(entries):
                        attrib_stationID = entry[0]
                        # attrib_arrival   = entry[1]
                        # attrib_departure = entry[2]
                        # attrib_stoptype  = entry[3]
                        
                        
                        if attrib_stationID in vrt:
                            firstonline = attrib_stationID               
                            first_sIDinVRT = n
                            break
                ...
                if condition: 
                    ...
                    if cbd not in stationIDs:
                        print(f'{tn} does not run to an accepted CBD location, sort trip in PTT using virtual run times to Central')
                        for entry in entries:
                            ...
                            if entry.attrib['stationID'] in vrt:
                                firstinline       = attrib_stationID
                                firstdeparture    = attrib_departure
                    ...
                    for entry in entries:
                        ...
                        ###!!!!! arrival times on final station
                        if stationType == 'pass':
                            tripdict[stationID] = 'exp'
                        elif stationID in ['MOH','EUD','WOB','PAL',last_listed_station]:
                    ...
                    tripdict['AM/PM'] = 'am' if odep < '12:00:00' or odep > '24:00:00' else 'pm'
            ...
            def refine_triplist(triplist, stations):
                ...
                for tripdict in triplist:
                    ...
                    # On the first loop we can obviously skip looking duplicates and just add the trip to the list
        ...
        # =============================================================================
            ...
            for x in trains:
                entries = []
                platform = re.findall(platformstring, x)
                # print(platform)
                ...
                for i,y in enumerate(splitlist):
                    if y.startswith('STRT'):
                        ...
                        sID = re.findall(platformstring, y)[0]
                        ...
                        arr = arrdep[:5]
                        dep = arrdep[5:]
                        arr = time_reformat(arr)
                        dep = time_reformat(dep)
                        ...
                        if i == 1:
                            oID = re.findall(stationstring, y)[0]
                            odep = dep
