...
# NBY (Normanby) removed from here - it now has its own explicit before/after
# mapping in TRACK_ID_OVERRIDES below instead of a blanket "T" prefix.
TIMING_POINT_OVERRIDES = {"CEN": "T", "MYJ": "T", "YLE": "T"}  # Caboolture North

# Explicit trackID overrides applied *after* the normal U-<platform>/D-<platform>
# (or T-<platform> for TIMING_POINT_OVERRIDES stations) value has been built.
# Keyed by station_id. Value is either:
#   - a dict of {computed_track_id: replacement_track_id} - only those exact
#     computed values are swapped, anything else for that station passes
#     through unchanged, or
#   - a plain string - every trackID for that station is forced to this
#     value regardless of what was computed.
TRACK_ID_OVERRIDES = {
    "NBY": {"U-1": "D-115", "D-2": "T-2"},  # Normanby
    "YLY": "T-4",  # all YLY trackIDs, regardless of direction/platform
    "BWH": {"U-1": "U-3", "D-2": "D-3"}
}


def apply_track_id_override(station_id, track_id):
    override = TRACK_ID_OVERRIDES.get(station_id)
    if override is None:
        return track_id
    if isinstance(override, dict):
        return override.get(track_id, track_id)
    return override
...
def build_entries_for_path(row, path):
    ...
    for i, (code, running_time) in enumerate(stations):
        ...
        track_id = apply_track_id_override(station_id, track_id)
        is_last = i == n - 1
        entries.append(
            {
